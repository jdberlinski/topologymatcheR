...
classify <- function(train, test, freqs = c(5, 10, 20), df = 5, samecol = TRUE) {
  ...
  fit_nonmatch <- MLmatrixt_ar1(corr_train$corrs[,, corr_train$matches == "nonmatch"], df = df, samecol = samecol) # nolint
...
#' Generating correlations from all read data, no base/tip
#'
#' this function finds the correlations between base and tip along all the knifes, image repetitions, and desired frequencies.
#'
#' This function creates a data frame which has columns containing some quantity (usually correlation) calculated for every pair
...
#' @author Carlos Llosa-Vite, \email{llosacarlos2@@gmail.com}
#' @author Geoffrey Z. Thompson
create.df.single <- function(data,
                      cuts,
                      freqs,
                      freq_mult = 1.775,
                      map_func = freq.map.radial,
  ...
  ## cleaned by cll 23/09/21, now I ignore all the "corclass" and "adjust"
  # original by gzt
  imgs <- 1:unname(dim(data[[1]])[3])
  d <- unname(dim(data[[1]])[1:2])
  if (is.numeric(cuts))
  {
    freqs <- cbind(cuts[1:(length(cuts)-1)],cuts[2:length(cuts)])
  }
  ...
  img.no <- dim(data[[1]])[3]
  nonmatch_names <- character()
  # match_vec <- rep(FALSE, length(knife.no) * (length(knife.no) - 1) * img.no)
  match_vec <- rep(FALSE, choose(length(knife.no), 2) * img.no)

  ##correlation matrix to store vectors of correlations at different frequency ranges
  # cormat <-
  #   matrix(nrow = length(knife.no) * (length(knife.no) - 1) * img.no,
  #          ncol = nrow(freqs))
  cormat <-
    matrix(nrow = choose(length(knife.no), 2) * img.no,
           ncol = nrow(freqs))
  ...
  for (i in 1:nrow(freqs))
  {
    ...
    for (j in 1:(length(data) - 1))
    {
      for (k in j:length(data))
      {
        if (j != k)
        {
          ...
          nonmatch_array <- array(dim = c(2, dim(data[[j]])))
          nonmatch_array[1, , , ] <- data[[j]]
          nonmatch_array[2, , , ] <- data[[k]]
          temp <-
            apply(
              X = nonmatch_array,
              MARGIN = 4,
    ...
    cormat[, i] <- c(match_cor, nonmatch_cor)
    # rownames(cormat) <- c(names(match_cor), names(nonmatch_cor))
    rownames(cormat) <- names(nonmatch_cor)
  ...
  cordf <- as.data.frame(cormat, row.names = 1:nrow(cormat))
  cordf$knife <- row.names(cormat)
  # cordf$img <- rep(imgs, times = length(knife.no) * (length(knife.no) - 1))
  cordf$img <- rep(imgs, times = choose(length(knife.no), 2))
  #set up match
  match_vec[match_vec == TRUE ] <- "match"
  match_vec[match_vec == FALSE] <- "nonmatch"
  # match_vec <- as.factor(match_vec)
